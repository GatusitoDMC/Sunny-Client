                ...
                for (setting in module.settings.getAll()) {
                    ...
                    when (setting) {
                        ...
                        is NumberSetting -> {
                            ...
                            context.drawText(
                                textRenderer,
                                "${setting.name}: %.1f".format(setting.value),
                                x + 4,
                                settingY,
                                0xFFFFFFFF.toInt(),
                                false
                    ...
                    for (setting in module.settings.getAll()) {
                        ...
                        if (mouseX >= xOffset + 2 && mouseX <= xOffset + width - 2 &&
                            ...
                            when (setting) {
                                ...
                                is ModeSetting<*> -> {
                                    val next = (setting.modes.indexOf(setting.value) + 1) % setting.modes.size
                                    @Suppress("UNCHECKED_CAST")
                                    (setting as ModeSetting<Enum<*>>).setMode(setting.modes[next] as Enum<*>)
                                    ConfigManager.save()
                                }
