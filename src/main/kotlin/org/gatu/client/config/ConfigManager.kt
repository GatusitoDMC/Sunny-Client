...
object ConfigManager {
    ...
    @Volatile var suppressSave: Boolean = false
        ...
        ModuleManager.getModules().forEach { module ->
            ...
            module.settings.getAll().forEach { setting ->
                when (setting) {
                    ...
                    is ModeSetting<*> -> settingsObject.addProperty(setting.name, (setting.value as Enum<*>).name)
    ...
    fun load() {
        ...
        try {
            ...
            if (root.has("friends")) {
                ...
                array.forEach { FriendManager.add(it.asString) }
            }

            ModuleManager.getModules().forEach { module ->
                if (!root.has(module.name)) return@forEach
                val moduleObject = root.getAsJsonObject(module.name)
                ...
                if (!moduleObject.has("settings")) return@forEach
                ...
                module.settings.getAll().forEach { setting ->
                    ...
                    when (setting) {
                        ...
                        is ModeSetting<*> -> setting.setModeByName(settingsObject.get(setting.name).asString)
